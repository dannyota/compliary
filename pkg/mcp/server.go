...
const defaultServerVersion = "0.1.17"
...
func NewServer(core *Core, log *slog.Logger, opts ...ServerOption) *Server {
	...
	mcpsdk.AddTool(srv, &mcpsdk.Tool{
		...
		Name:        "guide",
		Description: "Playbook for using compliary's evidence tools: scope, citation forms per framework, version-pin semantics, mapping-edge semantics, gaps philosophy, query tips including the framework filter's recall advantage (~88% vs ~72%).",
	}, s.handleGuide)
...
func buildInstructions(core *Core) string {
	base := "compliary is an evidence-only knowledge base for InfoSec & Cybersecurity control frameworks " +
		...
		"Use the framework filter for higher recall (~88% vs ~72% unfiltered)."
