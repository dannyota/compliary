...
compliary's PostgreSQL schema — banhmi's conventions ([`SCHEMA.md`](https://github.com/dannyota/banhmi)
patterns ported, crawl machinery dropped): sqlc, no cross-schema FKs (cross-layer links are
business-key `BIGINT`s), JSONB for non-queryable data, single-column surrogate PKs, natural keys as
composite `UNIQUE`, named constraints ≤63 bytes.
...
|-------|------|-------------|--------|
| `framework` | One row per framework | `code` (`iso27001`, `soc2tsc`, `pcidss`, `nistcsf`, `nist80053`, `ciscontrols`, `iso27002`, `iso27017`, `iso27018`, `iso27701`, `iso22301`, `iso42001`, `swiftcscf`, `csaccm`, `cobit`) · `name` · `publisher` · `source_access` (`auto-fetch`/`form-gated`/`byo`) · `license_class` (`public-domain`/`open-restricted`/`licensed`) · `citation_scheme` (parser + cite-format key) | `(code)` |
...
| `mapping_source` | Provenance vocab for mapping edges | `code` (`nist-olir`, `csa-ccm-v4.1`, `cis-v8.1-mappings`, `operator`) · `name` · `authority_note` | `(code)` |
...
|-------|------|-------------|
| `manifest_file` | One row per file under `data/` | `rel_path` UNIQUE · `sha256` · `size_bytes` · `framework_code` + `version_label` (matched from registry patterns; NULL = unrecognized → a quality gap) · `file_format` (`oscal-json`/`xlsx`/`pdf`) · per-stage state: `extracted_at`/`normalized_at`/`indexed_at` + `stage_error` · re-run diff: state resets when `sha256` changes |

Completeness = every `manifest_file` row for an `is_current` version reaching `indexed_at`,
recomputed by query — never a stored boolean.
...
| Table | Role | Notes |
|-------|------|-------|
| `document` | One publication (framework version's document) | `doc_key` = `<framework_code>|<version_label>|<doc_role>` (`main`/`amendment`/`companion-workbook`/`changelog`) · title · `source_file_sha256` business key · denormalized display `markdown` |
| `control` | **The citation unit** (control / clause / criterion / requirement / objective / safeguard) | `document_id` · `citation` UNIQUE per document (`A.5.1`, `AC-2(3)`, `CC6.1`, `8.3.6`, `PR.AA-01`, `CLD.6.3.1`, `AIS-01`, `EDM01`, `5.1`) · `citation_norm` (uppercased, separator-normalized — the mapping join key) · `parent_control_id` (tree: family → control → enhancement; clause → subclause) · `kind` (`family`/`control`/`enhancement`/`clause`/`criterion`/`requirement`/`objective`/`safeguard`/`annex-control`) · `title` (ours or publisher's) · `body` (verbatim normative text — **auth-gated at serve time by the document's `serve_gate`**) · `ordinal` |
| `version_relation` | Version lineage edges | `from_framework_code`+`from_version` → `to_framework_code`+`to_version` · `relation_type` (`supersedes`/`amends`/`consolidates`) · `note` (e.g. "cancels and replaces; realigned to 27002:2022") — populated from registry knowledge + publisher forewords |
| `control_mapping` | **Cross-framework edges, the product's second half** | `from_control_id` → `to_framework_code` + `to_citation_norm` (business key — target may not be ingested yet, banhmi's `doc_ref` stub pattern flattened) · `mapping_source_code` (config vocab) · `relationship` (`equivalent`/`subset-of`/`superset-of`/`intersects`/`related`) · `provenance_detail` (row/cell reference in the mapping workbook) · resolved `to_control_id` nullable, filled when the target framework lands |
...
| Table | Role | Notes |
|-------|------|-------|
| `chunk` | One control → one chunk | `control_id` · `citation` (display cite: `ISO/IEC 27001:2022 A.5.1`, `NIST SP 800-53r5 AC-2(3)`) · `context_prefix` (framework + version + family/clause path + title — deterministic contextual retrieval) · `content` · `content_sparse` (`sparsevec` BM25) · UNIQUE `(control_id)` |
| `chunk_embedding` | Dense vectors | Qwen3-Embedding 1024-d · one row per `(chunk_id, model)` · HNSW cosine |

Serve-time rule: `search` returns citations/titles/scores for everything, but chunk `content` for
`auth-only` documents only past auth — public callers get the citation + a gap notice, never the
licensed text.
...
1. **Manifest, not crawler** — `ingest` is one table; sha256 diff drives re-runs.
2. **`control` is the atom** — every framework's citation unit normalizes into one tree-shaped
   table discriminated by `kind`; the citation string is the natural key.
3. **Mappings are business-keyed** (`to_citation_norm`), resolve lazily — mapping workbooks load
   before/without the target framework's text (mirrors banhmi's `doc_ref` stubs).
4. **License gate lives on bronze, enforced at serve** — `serve_gate` flows bronze → silver
   document → MCP; public callers never see licensed `body`/`content`.
5. **Version currency is registry + edges** — no bitemporal machinery; `is_current` +
   `version_relation` covers the framework world's actual dynamics.
